...
public class ExcelExporter
{
    ...
    public void ExportSheets<T>(List<(string, List<T>)> items, string filePath) where T : class
    {
        ...
        foreach (var (sheetName, sheetItems) in items)
        {
            ...
            ExportCore(sheetItems, worksheetPart, CollectionsMarshal.AsSpan(PropertyCache.GetCachedProperties(typeof(T))));
    ...
    public void ExportSheet<T>(List<T> items, string fileName) where T : class
    {
        using var stream = new FileStream(fileName, FileMode.Create);
        ExportSheet(items, stream);
    }
    ...
    public void ExportSheet<T>(List<T> items, Stream stream) where T : class
    {
        using var spreadsheet = SpreadsheetDocument.Create(stream, SpreadsheetDocumentType.Workbook);
        ExcelPackageHelper.GenerateWorkbook(spreadsheet);
        WorksheetPart worksheetPart = spreadsheet.WorkbookPart!.AddNewPart<WorksheetPart>();
        ExportCore(items, worksheetPart, CollectionsMarshal.AsSpan(PropertyCache.GetCachedProperties(typeof(T))));
